...
-- 3. Agrega una nueva columna "rating" a la tabla "movies". El campo debe permitir numeros de 2 digitos y 1 decimal, y estar restringido sólo a números entre 0 y 10.alter
ALTER TABLE movies
ADD COLUMN rating DECIMAL(3, 1) CHECK (
    rating >= 0
    AND rating <= 10
);
...
INSERT INTO countries (country_name) VALUES ('España');
...
ALTER TABLE movies
ADD CONSTRAINT movies_fk_studios FOREIGN KEY (studio_id) REFERENCES studios (studio_id);

SHOW TABLES LIKE 'movie_review';
